...
const APP_UNPUBLISHED = `<!doctype html><meta charset="utf-8"><body style="background:#0E1525;color:#9DA2A6;font-family:system-ui;display:grid;place-items:center;height:100vh;margin:0"><div style="text-align:center"><p style="font-size:15px;color:#F5F9FC;margin-bottom:6px">This app is not published.</p><p>Publish it from the Replica workspace to give it a stable URL.</p></div>`;

const BRIDGE_TAG = '<script src="/replica-bridge.js"></script>';

/**
 * Serve preview HTML with the error bridge injected, so runtime errors in
 * the generated app surface in the workspace. The bridge must load BEFORE
 * any project script: errors thrown during initial execution are lost if
 * the listener registers later. Statically served files only; proxied
 * responses from a running process are passed through untouched.
 */
async function servePreviewHtml(res, full) {
  try {
    let html = await fsp.readFile(full, 'utf8');
    if (/<head[^>]*>/i.test(html)) html = html.replace(/<head[^>]*>/i, `$&${BRIDGE_TAG}`);
    else if (/<body[^>]*>/i.test(html)) html = html.replace(/<body[^>]*>/i, `$&${BRIDGE_TAG}`);
    else if (/^\s*<!doctype[^>]*>/i.test(html)) html = html.replace(/^\s*<!doctype[^>]*>/i, `$&${BRIDGE_TAG}`);
    else html = BRIDGE_TAG + html;
...
async function route(req, res, url) {
  ...
  if (pm) {
    ...
    if (/\.html?$/i.test(full)) return servePreviewHtml(res, full);
    return serveFile(res, full, { 'Cache-Control': 'no-store' });
  }

  // published apps: a stable URL that outlives the workspace session
  const am = p.match(/^\/apps\/([a-z0-9-]+)(\/.*)?$/);
  if (am) {
    const dir = store.projectDir(am[1]);
    const meta = dir ? await store.readMeta(dir) : null;
    if (!dir || !meta || !meta.published) {
      return send(res, 404, APP_UNPUBLISHED, { 'Content-Type': 'text/html; charset=utf-8' });
    }
    const up = proc.upstream(am[1]);
    if (up) return proxyRequest(req, res, up.port, encodeURI(am[2] || '/') + url.search);
    let rel = (am[2] || '/').replace(/^\/+/, '');
    if (!rel) rel = 'index.html';
    let full = safeJoin(dir, rel);
    if (!full) return send(res, 400, 'Bad path');
    if (fs.existsSync(full) && fs.statSync(full).isDirectory()) full = path.join(full, 'index.html');
