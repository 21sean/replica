 ...
 * the generated app surface in the workspace. Statically served files only;
 * proxied responses from a running process are passed through untouched.
 */
async function servePreviewHtml(res, full) {
  try {
    let html = await fsp.readFile(full, 'utf8');
    if (/<\/body>/i.test(html)) html = html.replace(/<\/body>/i, `${BRIDGE_TAG}</body>`);
