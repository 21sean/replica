...
- To run a console command and see its output, output on its own line: <<<RUN: command>>>
  Allowed commands start with node / python / npm, no shell chaining. Your RUN commands execute after your reply finishes, then you are called again with their output so you can fix problems or confirm success. You get a limited number of these rounds per user request, so run purposeful checks (like a script you just wrote). Never RUN a long-lived server; the user starts servers with the Run button.
...
- If the user needs a backend, write a Node (built-in http module) or Python server that reads the PORT environment variable and listens on 127.0.0.1. The user starts it with the Run button and the preview proxies to it. Do not hardcode ports.
- Use relative asset paths ("style.css", not "/style.css"). This matters for both static previews and proxied server apps.
- Zero external network dependencies: no CDNs, no Google Fonts, no external images, no npm installs. Use system font stacks, inline SVG, CSS gradients, and emoji.
 ...
 *
 * events: { narration(text), fileStart(path), fileChunk(path, bytes, text),
 *           fileDone(path, content, truncated), del(path), run?(command) }
 */
function createAgentParser(ev) {
  ...
  function pump() {
    for (;;) {
      if (!inFile) {
        const first = [
          { m: buf.match(FILE_RE), kind: 'file' },
          { m: buf.match(DEL_RE), kind: 'del' },
          { m: buf.match(RUN_RE), kind: 'run' },
        ].filter((c) => c.m).sort((a, b) => a.m.index - b.m.index)[0];
        if (first) {
          const { m, kind } = first;
      ...
      if (buf.length > hold) {
        const delta = buf.slice(0, buf.length - hold);
        fileBuf += delta;
        buf = buf.slice(buf.length - hold);
        ev.fileChunk(filePath, fileBuf.length, delta);
      }
      return;
    }
  }
